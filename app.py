...
with tab3:
    ...
    st.info("Para visualizar a imagem do mapa diretamente aqui na aplicação, coloque o arquivo `mapa_de_barovia.png` na mesma pasta do repositório no GitHub!")
    try:
        from PIL import Image
        image = Image.open("mapa_de_barovia.png")
        st.image(image, caption="Mapa de Baróvia (Escala: 1 Hexágono = 0.25 milhas)", use_column_width=True)
    except Exception:
        st.warning("Arquivo de imagem `mapa_de_barovia.png` não encontrado localmente. O mapa completo estará visível assim que você subir a imagem no GitHub.")
